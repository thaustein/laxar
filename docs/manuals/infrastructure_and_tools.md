...
The only exception to this rule is if the user triggers a _navigation_, indirectly through a widget, or directly by changing a URL in the browser.
...
If navigating to a new page, the startup process (described above) re-used, starting at step 3.
...
If your application causes a lot of HTTP requests for widget templates and CSS, it is likely to a misconfiguration of the file listings.
...
Most modern single page applications are no longer developed using just a text editor. 
...
It consists of several [grunt](http://gruntjs.com) tasks that help to manage the assets and dependencies used by your application, as well as a development server.
The application template has a grunt configuration file that will work for most scenarios, but if you want to add your own tools to the build pipeline, feel free to modify the `Gruntfile.js`.


### LaxarJS Grunt Tasks

Following are the most important LaxarJS grunt tasks. 
To actually run the tasks, you will usually use one of the short, _alias tasks_ defined to the end of the `Gruntfile.js` (see below). 
None of them is strictly necessary to build and run your application, but in concert they go a long way to help reduce boilerplate code and to allow for an optimized user experience.
In other words: you will not want to do without them.
  ...
  * `css_merger`:
    ...
    Similar to `portal_angular_dependencies`, this tasks finds all CSS for theme, widgets, layouts and controls within your application.
    It repeats the process once for each theme under `includes/themes`.
    The resulting theme CSS is then used during production, to have as few CSS `link` elements as possible, which reduces the number of HTTP requests as well the CPU load.
  ...
  * `widgets`:
    ...
    Runs all widgets' spec test in a headless web browser, using [Karma](http://karma-runner.github.io) and [PhantomJS](http://phantomjs.org/).
    This is a very useful starting point to setup continuous integration for your project, and for quickly checking your changes before making a commit during development.
