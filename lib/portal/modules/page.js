...
define( [
   ...
   module.controller( 'laxar.portal.PageController', [
      ...
      function( $scope, pageService, $q, configuration, layoutLoader, eventBus, fileResourceProvider, themeManager ) {
         ...
         var areaVisibilityManager = createAreaVisibilityManager();
         ...
         function setupPage( pageName ) {
            ...
            var pagePromise = pageLoader_.loadPage( pageName )
               ...
               .then( beginLifecycle )
               .then( areaVisibilityManager.initialize )
          ...
          */
         function createAreaVisibilityManager() {

            var ROOT = '';
            var subscriberOptions = { subscriber: 'AreaVisibilityManager' };
            var eventOptions = { sender: 'AreaVisibilityManager', deliverToSender: false };
            ...
            function initialize() {
               ...
               eventBus.subscribe( 'changeAreaVisibilityRequest', function( event ) {
                  return performAreaChange( event.area, event.visible );
               }, subscriberOptions );

               handleAreaChange( ROOT, true );
            }
            ...
            function requestAreaChange( area, visible ) {
               ...
               return eventBus.publishAndGatherReplies( request, event, eventOptions ).then( function( responses ) {
                  if( responses.length === 0 ) {
                     ...
                     return performAreaChange( area, visible );
                  }
                  else {
                     // assume the first 'did'-response to be authoritative:
                     var response = responses[ 0 ];
                     var logMessage = 'Propagating visibility state [0] for area [1] determined by [2]';
                     log.develop( logMessage, response.event.visible, area, response.meta.sender );
                     return handleAreaChange( area, response.event.visible );
                  }
               } );
            }
            ...
            function performAreaChange( area, visible ) {
               var will = [ 'willChangeAreaVisibility', area, visible ].join( '.' );
               var event = { area: area, visible: visible };
               eventBus.publish( will, event, eventOptions )
                  .then( function() {
                     return handleAreaChange( area, visible );
                  } )
                  .then( function() {
            ...
            function handleAreaChange( ofArea, areaVisible ) {
   ...
   function createAreaHelper( q, page ) {
      ...
      function attachWaitingAdapters( areaName ) {
         ...
         areaToWaitingAdapters[ areaName ].forEach( function( adapter ) {
      ...
      return {
         setVisibility: function( areaName, visible ) {
            if( areaName.indexOf( 'myPopup' ) === 0 ) {
               console.log( 'setV', areaName, visible ); // :TODO: Delete
            }

            visibilityByArea[ areaName ] = visible;
            if( visible && areaName in areaToWaitingAdapters ) {
               console.log( 'attaching', areaName ); // :TODO: Delete
         ...
         register: function( name, element ) {
            ...
            if( name in areaToWaitingAdapters && visibilityByArea[ name ] ) {
         ...
         attachWidgets: function( widgetAdapters ) {
            widgetAdapters.forEach( function( adapter ) {
               ...
               if( areaElement && visibilityByArea[ areaName ] ) {
