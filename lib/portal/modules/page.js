...
define( [
   ...
   '../paths'
], function( ng, $, storage, object, log, layoutModule, pageLoader, widgetLoaderModule, paths ) {
   'use strict';
   ...
   module.factory( 'axVisibilityService', [ 'axHeartbeat', function( heartbeat, $rootScope ) {
      ...
      function updateState( area, targetState ) {
         console.log( 'updateState:', area, targetState ); // :TODO: Delete
      ...
      function addHandler( id, area, handler, targetState ) {
         ...
         if( area in knownState && knownState[ area ] === targetState ) {
            console.log( 'addHandler(', id, area, handler, targetState, ')' ); // :TODO: Delete
            handler( targetState );
         }
      }
   ...
   module.controller( 'axPageController', [
      '$scope', 'axPageService', 'axVisibilityService', '$q', 'Configuration', 'LayoutLoader', 'EventBus', 'FileResourceProvider', 'ThemeManager', '$timeout',
      function( $scope, pageService, visibilityService, $q, configuration, layoutLoader, eventBus, fileResourceProvider, themeManager, $timeout ) {
         ...
         function setupPage( pageName ) {
            ...
            var layoutReady = layoutDeferred.promise.then( function( result ) {
               // function wrapper is needed: `renderlayout` must be accessed *after* the layout is ready
               renderLayout( result );
            } );
         ...
         /**
          * The VisibilityManager initializes and coordinates events for widget area visibility.
          *
         ...
         function createVisibilityManager() {
            ...
            var eventBusId = 'VisibilityManager';
            var ROOT = '';
            var subscriberOptions = { subscriber: eventBusId };
            var eventOptions = { sender: eventBusId, deliverToSender: false };
   ...
   /**
    * An area helper manages widget areas, their DOM representation and their nesting structure.
    *
    * It tracks widget area visibility to compile widgets and to attach them to their areas when these
    * become visible. It also tells the visibility service when change handlers need to be run. It does not
    * interact with the event bus directly, but is used by the visibility manager to determine area nesting
   ...
   function createAreaHelper( q, page, visibilityService ) {
      ...
      // the dom element for each area, to attach widgets to
      var areaToElement = {};
