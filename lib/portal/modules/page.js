...
define( [
   ...
   module.controller( 'laxar.portal.PageController', [
      ...
      function( $scope, pageService, $q, configuration, layoutLoader, eventBus, fileResourceProvider, themeManager ) {
         ...
         function createAreaVisibilityManager() {
            ...
            function initialize() {
               var topAreas = areaHelper_.containerToChildren( '' );
               topAreas.forEach( showArea );
            }

            function showArea( area ) {
               eventBus.publishAndGatherReplies( 'changeVisibilityRequest.' + area ).then( function( responses ) {
   ...
   function createAreaHelper( q, page ) {
      ...
      var containerToChildren = {};
      object.forEach( page.areas, function( widgets, areaName ) {
         widgets.forEach( function( widget ) {
            var containerName = '';
            if( areaName.indexOf( '.' ) === -1 ) {
               var containerWidgetId = areaName.split( '.' )[ 0 ];
               containerName = widgetIdToArea[ containerWidgetId ];
            }
            containerToChildren[ containerName ] = containerToChildren[ containerName ] || [];
            containerToChildren[ containerName ].push( name );
         } );
      } );

      return {
         containerToChildren: function( containerName ) {
            return containerToChildren[ containerName ];
         },
         /**
          * @param {String} name
