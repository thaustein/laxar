...
define( [
   ...
   '../paths',
   '../timer'
], function( ng, $, storage, object, log, layoutModule, pageLoader, widgetLoader, paths, timer ) {
   'use strict';
   ...
   module.run( [
      ...
      function( $q, layoutLoader, eventBus, fileResourceProvider, themeManager ) {
         ...
         pageLoader_ = pageLoader.create( $q, null, paths.PAGES, fileResourceProvider );
   ...
   module.service( 'portal.PageService', [ function( ) {
      ...
      return {
         /**
          * Instantiate all widget controllers on the given page, and then load their UI.
          *
          * @return {Promise}
          *    A promise that is resolved when all controllers have been instantiated.
          *    It is resolved with a function that performs the actual DOM-construction when invoked.
          */
         setupPage: function( page ) {
            // instantiate controllers
            var widgets = widgetsForPage( page );
            var adapterPromises = widgets.map( function( widget ) {
               return widgetLoader.load( widget );
            } );
