...
define( [
   ...
   module.controller( 'laxar.portal.PageController', [
      ...
      function( $scope, pageService, $q, configuration, layoutLoader, eventBus, fileResourceProvider, themeManager ) {
         ...
         function createAreaVisibilityManager() {
            ...
            function initialize() {
               var topAreas = areaHelper_.childrenOf( '' );
               topAreas.forEach( show );
            }

            //////////////////////////////////////////////////////////////////////////////////////////////////

            function show( area ) {
               return requestChange( area, true );
            }

            //////////////////////////////////////////////////////////////////////////////////////////////////

            function hide( area ) {
               return requestChange( area, false );
            }

            //////////////////////////////////////////////////////////////////////////////////////////////////

            /** First, ask if someone is responsible for managing this area's visibility. */
            function requestChange( area, visible ) {
               var request = [ 'setVisibilityRequest', area ].join( '.' );
               ...
               return eventBus.publishAndGatherReplies( request, event, eventOptions ).then( function( responses ) {
                  if( responses.length === 0 ) {
                     ...
                     return setRecursively( area, visible );
                  }
                  else {
                     // assume the first 'did'-response to be authoritative:
                     var response = responses[ 0 ];
                     console.log( 'Received visibility state for area ', area, ': ', response ); // :TODO: Delete
                     console.log( '', responses[ 0 ] ); // :TODO: Delete

                     // :TODO: propagate visibility for nested areas based on the respone
                     return $q.when();
                  }
               } );
            }
            ...
            function setRecursively( area, visible ) {
               var will = [ 'willSetVisibility', area, visible ].join( '.' );
               var event = { area: area, visible: visible };
               eventBus.publish( will, event, eventOptions )
                  .then( function() {
                     var children = areaHelper_.childrenOf( area );
                     if( !children ) { return $q.when(); }
                     return $q.all( children.forEach( visible ? show : hide ) );
                  } )
                  .then( function() {
                     var did = [ 'didSetVisibility', area, visible ].join( '.' );
   ...
   function createAreaHelper( q, page ) {
      ...
      // for each widget area with children, store a list of child names
      var childrenOf = {};
      object.forEach( page.areas, function( widgets, areaName ) {
         widgets.forEach( function( widget ) {
            var containerName = '';
            ...
            childrenOf[ containerName ] = childrenOf[ containerName ] || [];
            childrenOf[ containerName ].push( areaName );
         } );
      } );

      return {
         childrenOf: function( containerName ) {
            return childrenOf[ containerName ];
         },
         /**
          * @param {String} name
