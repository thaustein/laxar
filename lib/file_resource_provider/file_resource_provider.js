...
define( [
    ...
    *
    * @param {String} url the uri to the resource to provide
   ...
   FileResourceProvider.prototype.provide = function( url ) {
      var self = this;
      return isListed( this, url ).then( function( knownListed ) {
         return knownListed ? httpGet( self, url ).then( resourceTransform( url ) ) : q_.reject();
      }, function() {
   ...
   FileResourceProvider.prototype.isAvailable = function isAvailable( url ) {
      var self = this;
      return isListed( self, url ).then( function( knownListed ) {
         return q_.when( knownListed );
      }, function() {
   ...
   function isListed( provider, resourcePath ) {
      ...
      if( usablePrefixes.length ) {
         ...
         return fetchListingForPath( provider, prefix ).then( function( listing ) {
            return q_.when( findFileInListing( provider, resourcePath, listing ) );
         } );
      }
